...
def compile_typescript(ts_code: str) -> bool:
    ...
    try:
        ...
        result = subprocess.run(['tsc', '--target', 'es2016', '--module', 'commonjs', temp_file_path],
...
def batch_compile_typescript_to_file(
    ...
    # Compile each TypeScript file separately in parallel, skip if the code has already been compiled
    def compile_ts(ts_path):
        ...
        try:
            ...
            print("Error compiling TypeScript file: ", ts_path,
                  "\nwith error: ", e)
            return False
...
def test_javascript_batch(js_filenames: list[str]) -> list[bool]:
    ...
    def run_js(js_path):
        try:
            ...
            print("Error running TypeScript test: ", js_path,
                  "\nwith error: ", e)
            return False
