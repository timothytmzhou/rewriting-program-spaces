...
def compile_csv(csv_path):
    ...
    with open(csv_path, newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            ...
            subdir = os.path.join(os.getcwd(), 'benchmarks', 'mbpp_benchmarks', benchmark_name)
...
def update_function_names(csv_path):
    ...
    for idx, row in df.iterrows():
        ...
        func_name_path = os.path.join("/home/ubuntu/rewriting-program-spaces/experiments/typescript/benchmarks/mbpp_benchmarks/",
    ...
    for temp in ['0.01', '0.3', '0.5', '0.7', '1.0']:
        for mode in ['GCD', 'TypedCD', "Unconstrained"]:
            for model in ['llama13b', 'deepseek-coder']:
                update_function_names(f'/home/ubuntu/rewriting-program-spaces/experiments/typescript/results2/{mode}_temp_{temp}_model_{model}.csv')
        ...
        for mode in ['GCD', 'TypedCD', "Unconstrained"]:
            for model in ['llama13b', 'deepseek-coder']:
                results[(temp, mode, model)] = compile_csv(
                    f'/home/ubuntu/rewriting-program-spaces/experiments/typescript/results2/{mode}_temp_{temp}_model_{model}.csv')
        ...
        for mode in ['GCD', 'TypedCD', "Unconstrained"]:
            for model in ['llama13b', 'deepseek-coder']:
                results[(temp, mode, model)] = process_test_scripts(
                    f'/home/ubuntu/rewriting-program-spaces/experiments/typescript/results2/{mode}_temp_{temp}_model_{model}/')
                print(f"{temp}_{mode}_{model} =>{results[(temp, mode, model)]}")
